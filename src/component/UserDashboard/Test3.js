...
export default function ReceiptManager() {
  ...
  const saveReceipt = async () => {
    ...
    const { data } = await supabase.from("receipts").select("*").eq("sale_group_id", selectedSaleGroup.id).order('id', { ascending: false });
    setReceipts(data);
  };
                      ...
                      <div className="flex gap-3">
                        ...
                        <button
                          onClick={async () => {
                            ...
                            const { data } = await supabase.from("receipts").select("*").eq("sale_group_id", selectedSaleGroup.id);
                            setReceipts(data);
                          }}
